...
struct LocationValidatorTests {
    ...
    func testInvalidLocation_InvalidTimeZone() async throws {
        ...
        #expect(!result.isValid)
        #expect(result.errors.contains { error in
            if case .invalidValue(field: "time_zone", reason: "Invalid time zone identifier") = error { return true }
            return false
        })
    }
    ...
    func testInvalidLocation_InvalidConnector() async throws {
        ...
        #expect(result.errors.contains { error in
            if case .invalidValue(field: "evses[0].connectors[0].max_voltage", reason: "Must be greater than 0") = error { return true }
            return false
        })
        #expect(result.errors.contains { error in
            if case .invalidValue(field: "evses[0].connectors[0].max_amperage", reason: "Must be greater than 0") = error { return true }
            return false
        })
    }
} 