...
struct TokenValidatorTests {
  ...
  func testInvalidToken_InvalidRFIDFormat() async throws {
    ...
    #expect(
      result.errors.contains { error in
        if case .invalidValue(field: "uid", reason: "Invalid RFID uid format") = error {
          return true
        }
        return false
      })
    ...
    #expect(
      result.errors.contains { error in
        if case .invalidValue(
          field: "whitelist",
          reason: "Invalid tokens cannot have ALWAYS or ALLOWED_OFFLINE whitelist type") = error
        {
          return true
        }
        return false
  ...
  func testInvalidToken_InvalidLanguageCode() async throws {
    ...
    #expect(
      result.errors.contains { error in
        if case .invalidValue(field: "language", reason: "Invalid ISO 639-1 language code") = error
        {
          return true
        }
        return false
  ...
  func testAppUserToken_InvalidAuthId() async throws {
    ...
    #expect(
      result.errors.contains { error in
        if case .invalidValue(field: "auth_id", reason: "Invalid app user auth_id format") = error {
          return true
        }
        return false
      })
  ...
  func testAdHocUserToken_InvalidWhitelist() async throws {
    ...
    #expect(
      result.errors.contains { error in
        if case .invalidValue(
          field: "whitelist",
          reason: "Ad-hoc users cannot have ALWAYS or ALLOWED_OFFLINE whitelist type") = error
        {
          return true
        }
        return false
