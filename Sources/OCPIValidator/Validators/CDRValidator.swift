...
public struct CDRValidator {
  ...
  private func validateTotals(_ cdr: CDR, errors: inout [ValidationError]) {
    if cdr.totalCost < 0 {
      errors.append(
        .invalidValue(field: "total_cost", reason: "Must be greater than or equal to 0"))
    }

    if cdr.totalEnergy < 0 {
      errors.append(
        .invalidValue(field: "total_energy", reason: "Must be greater than or equal to 0"))
    }

    if cdr.totalTime < 0 {
      errors.append(
        .invalidValue(field: "total_time", reason: "Must be greater than or equal to 0"))
    }

    if let parkingTime = cdr.totalParkingTime, parkingTime < 0 {
      errors.append(
        .invalidValue(field: "total_parking_time", reason: "Must be greater than or equal to 0"))
    }
